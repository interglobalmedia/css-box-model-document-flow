...
If the `position: absolute;` or `position: fixed;` ***property declaration*** is ***applied***, the `right property` ***sets*** the `right edge` of an `element` to a `unit` to the ***right*** of the `right edge` of its ***nearest*** `positioned ancestor`.
...
If the `position: sticky;` ***property declaration*** is ***applied***, the `top property` ***behaves*** like its `position` is `relative` ***when*** the `element` is ***inside*** the `viewport`, and like its `position` is `fixed` ***when*** it is ***outside*** the `viewport`.
